...
interface AuthScreenProps {
  onAuth: (user: { id: number; name: string; username: string; avatar: string; bio: string; status: string }) => void;
}
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (mode === 'register') {
        ...
        result = await api.register(form.name, form.username, form.password);
      } else {
      ...
      localStorage.setItem('vector_current_user', JSON.stringify(result.user));
      onAuth(result.user);
    } catch (err: unknown) {
        ...
        <div className="bg-card border border-border rounded-2xl p-6 shadow-2xl">
          <div className="flex bg-secondary rounded-xl p-1 mb-6">
            <button
              ...
              className={`flex-1 py-2 text-sm font-medium rounded-lg transition-all duration-200 ${mode === 'login' ? 'bg-primary text-primary-foreground shadow-sm' : 'text-muted-foreground hover:text-foreground'}`}
            >Вход</button>
            <button
              onClick={() => { setMode('register'); setError(''); }}
              className={`flex-1 py-2 text-sm font-medium rounded-lg transition-all duration-200 ${mode === 'register' ? 'bg-primary text-primary-foreground shadow-sm' : 'text-muted-foreground hover:text-foreground'}`}
            >Регистрация</button>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-3">
            ...
            <button
              ...
              className="w-full bg-primary hover:bg-primary/90 disabled:opacity-60 text-primary-foreground font-semibold py-2.5 rounded-xl transition-all duration-200 text-sm mt-2 hover:scale-[1.02] active:scale-[0.98] flex items-center justify-center gap-2"
            >
